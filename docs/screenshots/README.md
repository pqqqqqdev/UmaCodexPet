...
here only after they are cleared for publication; do not commit a generated pet
...
| `02-f6-animations-face.png` | F6 Animations/Face state page with Auto choices and live-preview status/reload control | Crop other desktop content; do not expose local paths |
...
| `07-export-complete.png` | UmaPetForge completion log beside `EXPORT_COMPLETE.txt` | Remove username, install path, and unrelated logs |
| `08-atlas-preview.png` | One atlas in a checkerboard transparency viewer | Confirm redistribution permission first |

After adding approved images, reference them from the relevant root README
section with relative Markdown links.
